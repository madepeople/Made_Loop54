...
class Loop54_Entity
{
    ...
    {
        if (!array_key_exists($key, $this->_attributes)) return null;
        return $this->_attributes[$key];
    }

    public function getStringAttribute($key)
    {
        if (!array_key_exists($key, $this->_attributes)) return "";
        $ret = "";
    ...
    {
        if (!array_key_exists($key, $this->_attributes)) $this->_attributes[$key] = array();
    ...
    public function serialize()
    {
        ...
        foreach ($this->_attributes as $key => $values) {
            ...
            foreach ($values as $value) {
                ...
                if (is_string($value)) $ret .= "\"" . Loop54_Utils::escape($value) . "\","; else if ($value instanceof DateTime) $ret .= "\"" . $value . "\","; else$ret .= $value . ",";
            }
            $ret = rtrim($ret, ',');
            $ret .= "],";
        }
