...
abstract class Loop54_Utils
{
    ...
    {
        $existingCookie = null;
        if (isset($_COOKIE{'Loop54User'})) $existingCookie = $_COOKIE{'Loop54User'};
        if ($existingCookie != null) return $existingCookie;
        $userId = str_replace(":", "", Loop54_Utils::getIP()) . "_" . Loop54_Utils::randomString(10);
        setcookie('Loop54User', $userId, time() + (86400 * 365), "/");
        $_COOKIE{'Loop54User'} = $userId;
        return $userId;
    }
    ...
    static function fixEngineUrl($url)
    {
        ...
        $url = strtolower($url);
        if (!Loop54_Utils::stringBeginsWith($url, "http")) $url = "http://" . $url;
        if (!Loop54_Utils::stringEndsWith($url, "/")) $url = $url . "/";
        return $url;
    }
